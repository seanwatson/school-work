...
const RGB maroon(128,0,0), green (0,128,0),    navy(0,0,128),     gray(128,128,128);
...
inline double dI(const RGB &a, const RGB& b) {return I(a)-I(b);}
...
template <class T>  // applies when T->RGB casting is defined for type "T" (see RGB class above); "pos" - position in a window, "alpha"- transparency.
void drawImage(const Table2D<T>& arr, Point pos = Point(), double alpha = 1.0); // arr's values outside [0,255] are truncated at 0 or 255. 

template <class T>  // saves Table2D of type "T" to BMP file, uses T->RGB cast orerator (if defined for RGB class above)
void saveImage(const Table2D<T>& arr, const char * bmp_file_name); // arr's values outside [0,255] are truncated at 0 or 255. 
...
template<class T, class ConversionToRGB>  // works for any type "T", but needs specialized functor "f" with  convert(RGB,T) method
void drawImage(const Table2D<T>& arr, const ConversionToRGB& f, Point pos = Point(), double alpha = 1.0);

template<class T, class ConversionToRGB>  // as above, but also takes an alpha-mat (transparancy table) with values in interval [0,1]
void drawImage(const Table2D<T>& arr, const ConversionToRGB& f, const Table2D<double>& alpha_mat, Point pos = Point());
               //PRECONDITION: "alpha-mat" must have the same size as "arr" and values between 0 and 1.

template<class T>  // works for integer-types "T" (like int, unsigned,...), requires look-up tables for RGB colors and transparancies (alpha-values in interval [0,1])
void drawImage(const Table2D<T>& arr, const RGB* palette, const double* alpha, Point pos = Point()); 
              // PRECONDITION: look-up tables palette[i] and alpha[i] must be valid for the whole range of integer values in "arr"
