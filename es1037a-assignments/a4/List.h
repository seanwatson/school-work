...
public: 
    ...
    Item replace (unsigned int pos, Item item); // replace the element at location pos in the list with item,
                                                ...
                                                // PRECONDITION: the list is not empty, and pos is between 1 and getLength();
    inline void append(Item item) {return insert(getLength()+1,item);} // adds an item at the back of the list
    inline Item back() const {return retrieve(getLength());} // retrieves an item from the back of the list
    inline Item popBack() {return remove(getLength());} // removes an item from the back of the list
